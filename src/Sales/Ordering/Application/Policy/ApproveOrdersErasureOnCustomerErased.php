<?php

declare(strict_types=1);

namespace Sales\Ordering\Application\Policy;

use Crm\Customer\Application\IntegrationEvent\CustomerErased\CustomerErasedIntegrationEvent;
use Patchlevel\EventSourcing\Attribute\Subscribe;
use Sales\Ordering\Application\Command\ApproveOrderErasure\ApproveOrderErasure;
use Sales\Ordering\Application\Finder\Order\OrderFinderInterface;
use Shared\Application\Command\CommandBusInterface;
use Shared\Application\Exception\ApplicationExceptionInterface;
use Shared\Application\Policy;

#[Policy('sales.ordering.approve_orders_erasure_on_customer_erased')]
final readonly class ApproveOrdersErasureOnCustomerErased
{
    public function __construct(
        private OrderFinderInterface $orderFinder,
        private CommandBusInterface $commandBus,
    ) {
    }

    /**
     * @throws ApplicationExceptionInterface
     * @throws \DomainException
     */
    #[Subscribe(CustomerErasedIntegrationEvent::class)]
    public function __invoke(CustomerErasedIntegrationEvent $event): void
    {
        foreach ($this->orderFinder->byCustomer($event->customerId) as $order) {
            $this->commandBus->dispatch(new ApproveOrderErasure($order->id));
        }
    }
}
