...
namespace Sales\Order\Application\Command\AuthorizeOrderPayment;
...
#[CommandUseCase]
final readonly class AuthorizeOrderPaymentHandler
{
     ...
     */
    public function __invoke(AuthorizeOrderPayment $command): void
    {
        $orderPayment = $this->repository->load(OrderPaymentId::fromString($command->id));
        $orderPayment->authorize($this->clock->now());
