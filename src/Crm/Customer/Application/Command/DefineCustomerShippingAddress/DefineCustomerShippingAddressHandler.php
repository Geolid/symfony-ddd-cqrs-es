...
#[CommandHandler]
final readonly class DefineCustomerShippingAddressHandler
{
     ...
     */
    public function __invoke(DefineCustomerShippingAddress $command): void
    {
        $customer = $this->repository->load(CustomerId::fromString($command->customerId));

        $customer->defineShippingAddress(
            PostalAddressMapper::fromArray($command->shippingAddress),
            $this->clock->now(),
        );
