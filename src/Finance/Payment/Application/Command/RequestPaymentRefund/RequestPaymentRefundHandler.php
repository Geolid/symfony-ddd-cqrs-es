...
#[CommandHandler]
final readonly class RequestPaymentRefundHandler
{
     ...
     */
    public function __invoke(RequestPaymentRefund $command): void
    {
        $payment = $this->repository->load(PaymentId::fromString($command->paymentId));
        $payment->requestRefund($command->refundId, $this->clock->now());
