...
final class AnonymizeExpiredOrdersCommandTest extends AbstractCliTestCase
{
    ...
    public function itAnonymizesPastTheRetentionPeriod(): void
    {
        ...
        self::assertNotNull($expiredResult->anonymizedAt);
        $withinRetentionResult = $this->orderFinder->ofId($withinRetention->id->toString());
        self::assertNull($withinRetentionResult->anonymizedAt);
    }
    ...
    {
        // Given
        $order = OrderBuilder::new()->withPlacedAt(Clock::get()->now()->modify('-11 years'))->create();
        $this->store($order);
