...
- A side effect needing the same transaction as its triggering command (no eventual-consistency tolerance — a failure there must roll back the trigger too) uses `#[Shared\Application\Processor\SyncProcessor(...)]` instead of `#[Processor]` — the base `patchlevel_event_sourcing.php` config runs its `sync_processor` group synchronously, same connection, in every environment. A reaction that only needs eventual consistency (dispatching a Command onward, a cross-BC cascade) stays on `#[Processor]`'s default group.
