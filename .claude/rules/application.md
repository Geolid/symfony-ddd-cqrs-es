...
- Pre-check a condition the target aggregate's own transition method already guards — call it unconditionally and let the aggregate's self-guard/exception decide (Tell, Don't Ask).
...
- A side effect needing the same transaction as its triggering command (no eventual-consistency tolerance — a failure there must roll back the trigger too) tags its `#[Processor]` with `group: Shared\Application\Processor\SyncProcessorGroup::NAME` instead of the default `processor` group; the base `patchlevel_event_sourcing.php` config runs that group synchronously, same connection, in every environment. A reaction that only needs eventual consistency (dispatching a Command onward, a cross-BC cascade) stays on the default group.
