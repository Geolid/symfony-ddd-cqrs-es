...
### Conventions
- `Infrastructure/`'s top-level folders name a technical concern (`Persistence/`, `Monitoring/`, `Security/`...), never a framework mechanism (no `Messaging/`, `HttpKernel/`) — the mechanism stays visible in the class name's own suffix instead (`SentryMessengerMiddleware`, `DbalTransactionMessengerMiddleware`), not in the path.
