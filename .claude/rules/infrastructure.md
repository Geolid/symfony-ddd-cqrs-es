...
- DBAL Finder: `Dbal<X>Finder extends AbstractDbalFinder<Result>` (`buildBaseQuery()`/`mapRow()`), `final`, never `readonly` (`$queryBuilder` is clone-reassigned). A single-shot lookup exposes `ofX(): ?Result` via `$this->query()`; a collection/filterable one additionally extends `AbstractDbalCollectionFinder` for `with*(): static` (`$this->filter()`), iteration, `paginate()`. Interfaces mirror the split: `FinderInterface` (marker) → `CollectionFinderInterface` → `PaginatedCollectionFinderInterface`.
