...
- Event Store repository: `Persistence/EventStore/Repository/<Agg>Repository`, `final readonly`, implements `<Agg>RepositoryInterface`; injects `#[Autowire(service: 'event_sourcing.<subdomain>.<bc>.<aggregate>.repository')] Repository<Agg> $repository`; `has()` delegates as-is, `load()` catches `AggregateNotFound` and re-throws its own `<Agg>NotFoundException`; `save()` delegates as-is, or, when the aggregate's own identity is the sole idempotency guard against a duplicate creation, catches `AggregateAlreadyExists` and re-throws its own `<Agg>AlreadyExistsException` the same way.
- Projector: `#[Projector('<subdomain>.<bc>.<name>')] final readonly Dbal<X>Projector extends AbstractDbalProjector` (e.g. `sales.order.orders`), a `public const string TABLE`, one `#[Subscribe(<Event>::class)] on<Event>()` method per event, and `configureSchema()`. A status/type column stores the Application `<X>Status` enum's value, never the Domain `<X>State` enum's — the projection is a read model consumed through a Finder/DM, the same boundary that already forbids the Domain enum crossing into `Application/`/`dm.md`'s exposition surface; deriving the column value from `<X>State` instead couples the stored/filterable value to a representation Domain never promises to keep in sync with `<X>Status`.
