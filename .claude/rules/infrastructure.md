...
- A pure computation over primitives a Projector already has in hand, zero I/O, zero stream fold, stays colocated with that Projector, named for what it specifically does (never `Reducer`/`Resolver` — both imply reading the event store) — no dedicated folder/role convention for this shape until a second real occurrence exists; inventing one from a single instance is premature.
