...
- A fold serving a use case rather than a projection is `Persistence/EventStore/Resolver/Stream<X>Resolver implements <X>ResolverInterface` (the port is declared in Application, named without the `Stream` prefix since it's mechanism-agnostic), same `Store::load()` + `Reducer` mechanism, returning a small immutable DTO or `null` — reached for only once a local Finder kept current by that BC's own Integration Events (`application.md`) has been ruled out, never as the default. Every folder under `Persistence/` names the *role* — `Repository/`, `Translator/`, `EventStore/Resolver/`, `Projection/Reducer/` — never a bare vendor mechanism name; the `Stream` prefix on the class distinguishes a fold over the event stream from a sibling case built over already-materialized data, and comes first (`Stream<X>Resolver`/`Stream<X>Reducer`, matching `Dbal<X>Finder`/`<Vendor><X>Gateway`'s own mechanism-first ordering), never as an infix.
