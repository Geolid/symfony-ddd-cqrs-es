...
- Input mapped to a Command/Query field is validated through the BC's own format-level compound constraint (`Application/Validation/Valid<Name>`) for that concept; a compound that also encodes a state-dependent business rule (a uniqueness check, an external lookup) is reused only where that rule's own semantics genuinely apply to the use case at hand, never merely because the field shares that concept's name. A param outside that model (a URI variable, a filter) gets an `Assert` guard before dispatch.
- A closed vocabulary the DM has to name (a status, a type) is the BC's `Application/<X>Status` enum, never the Domain `<X>State` enum — the Domain enum carries the aggregate's behaviour and `Domain/` is not part of a DM's exposition surface. Where the vocabulary isn't user input but the use case itself, the DM names no value at all: it dispatches a dedicated Query and the BC decides what the filter means.
...
- A web write goes through a Symfony Form: a `Form/<Concept>/<Concept>Type extends AbstractType<<Concept>FormData>` bound to a plain `Form/<Concept>/<Concept>FormData` (public typed properties, no logic) — never read straight off `$request->request`. The Type itself never reaches a bus: what a field needs (a choice list) is asked by the controller and handed over as a form option.
...
- A CLI command driven by a single external actor invocation (one call per real-world unit of work) takes that unit's identity as an `Argument` and skips `LockableTrait` — the target aggregate's own transition guard is the redelivery/reentry safety net. `LockableTrait` stays reserved for a command that scans and acts on a shared collection in one run, where a concurrent second run would double-dispatch across the same batch.
...
- The DTO shape follows the receiving mechanism: API write `Input/*Input` (API Platform `input:`), Web read needing a compound constraint or grouping 2+ fields `Controller/QueryString/<Action>QueryString` (`#[MapQueryString]`, `<Action>` named after the controller action consuming it, never the resource), Web read of a single scalar with no compound constraint to carry a bare `#[MapQueryParameter]` argument on the action method instead — no DTO class, Web form write `Form/<Concept>/<Concept>Type` + `Form/<Concept>/<Concept>FormData`, Web JSON write `Controller/Payload/*Payload` (`#[MapRequestPayload]`, CSRF required), CLI `Console/Input/*Input` (properties carry both `#[Argument]`/`#[Option]` and their validation constraints, bound via `#[MapInput]` on `__invoke()` — validated automatically before the method runs), Webhook `*Payload` — each carries the BC's own validation compounds.
- A `Controller/` stays one flat file per primary route concept. A route nested under another concept (a sub-page, a sub-resource) gets its own controller under `Controller/<ParentConcept>/`, named after the sub-concept alone, never prefixed with the parent's name again.
