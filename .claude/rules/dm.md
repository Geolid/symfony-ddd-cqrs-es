...
- The DTO shape follows the receiving mechanism: API write `Input/*Input` (API Platform `input:`), Web read `Controller/Criteria/*Criteria` (`#[MapQueryString]`), Web form write `Form/<X>Type` + `Form/FormData/*FormData`, Web JSON write `Controller/Payload/*Payload` (`#[MapRequestPayload]`, CSRF required), Webhook `*Payload` — each carries the BC's own validation compounds.
