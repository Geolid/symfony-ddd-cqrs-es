...
- A closed vocabulary the DM has to name (a status, a type) is the BC's `Application/Language/Published<X>` enum, never the Domain enum — the Domain enum carries the aggregate's behaviour and `Domain/` is not part of a DM's exposition surface. Where the vocabulary isn't user input but the use case itself (`fulfilment:shipment:dispatch-pending`), the DM names no value at all: it dispatches a dedicated Query and the BC decides what the filter means.
