...
- A closed vocabulary the DM has to name (a status, a type) is read from the compound's `public const array VALUES`, never from the Domain enum — the enum stays in Domain because the aggregate holds it, and `Domain/` is not part of a DM's exposition surface. Where the vocabulary isn't user input but the use case itself (`shipment:dispatch-pending`), the DM names no value at all: it dispatches a dedicated Query and the BC decides what the filter means.
...
- The DTO shape follows the receiving mechanism: API write `Input/*Input` (API Platform `input:`), Web read `Controller/Criteria/*Criteria` (`#[MapQueryString]`), Web form write `Form/<X>Type` + `Form/FormData/*FormData`, Web JSON write `Controller/Payload/*Payload` (`#[MapRequestPayload]`, CSRF required), CLI `Console/Input/*Input` (properties carry both `#[Argument]`/`#[Option]` and their validation constraints, bound via `#[MapInput]` on `__invoke()` — validated automatically before the method runs), Webhook `*Payload` — each carries the BC's own validation compounds.
