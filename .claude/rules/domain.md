...
- `#[Aggregate('<subdomain>.<bc>.<aggregate>')]`, three segments even when the aggregate shares its BC's name; `#[Event('<subdomain>.<bc>.<past-tense verb>')]` — same two-segment prefix, the aggregate segment replaced by the verb. That aggregate segment may instead be the Ubiquitous Language term for the concept the event itself is about — the word domain experts actually use, never a private aggregate-only nickname — when the aggregate's own name would stutter against the verb; the substitute term stays identical across every event on that aggregate, never chosen per event.
- An `#[Apply]` method name is `apply<Event>`, `<Event>` being the event's own class name with its aggregate-name prefix dropped (the method already lives inside that aggregate) — never a short verb unrelated to the event, never the full un-trimmed event class name. Where the `#[Event]` rule above substituted a Ubiquitous Language term for the aggregate segment, the dropped prefix follows that same term instead — never a different choice.
