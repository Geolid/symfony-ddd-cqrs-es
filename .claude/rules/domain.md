...
- A Value Object is `final readonly`; private constructor enforces invariants (`Webmozart\Assert`); a named constructor (`fromCents`, `fromString`...) validates the input shape. Expose `equals()`/`toString()` as needed — never a generic `toArray()`/`fromArray()` pair: an event/command field carrying a VO's data stays an array of its own primitive properties, mapped by hand at each construction site (`recordThat()`, `#[Apply]`, a Handler) — the array shape is that call site's concern, not the VO's.
