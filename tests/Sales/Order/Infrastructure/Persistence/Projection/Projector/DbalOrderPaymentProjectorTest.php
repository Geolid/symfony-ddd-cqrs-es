...
/**
 * @phpstan-type Row array{order_id: string, amount_in_cents: int|string, reference: string, status: string, captured_at: ?string}
 */
final class DbalOrderPaymentProjectorTest extends AbstractIntegrationTestCase
{
    ...
    {
        // When
        $orderPayment = OrderPaymentTestFactory::new()->withAmountInCents(4_200)->withReference('GLBX-9F3K2M1P')->create();
    ...
    private function fetchRow(string $id): array|false
    {
        ...
        return $this->serviceAs('doctrine.dbal.read_model_connection', Connection::class)->fetchAssociative(
            \sprintf(
                'SELECT order_id, amount_in_cents, reference, status, captured_at FROM %s WHERE id = :id',
