...
use Fulfilment\Shipment\Application\Carrier\ReturnManifestedShipmentReconciler;
...
final class ReturnManifestedShipmentReconcilerTest extends AbstractIntegrationTestCase
{
    ...
    public function itReconcilesWhenReturnDispatched(): void
    {
        ...
        $shipment = ShipmentTestFactory::new()
            ...
            ->create();
        $this->store($shipment);
        $reconciler = new ReturnManifestedShipmentReconciler(new StubCarrierGateway([$returnTrackingReference => ShipmentStatus::RETURN_DISPATCHED->value]), $this->service(CommandBusInterface::class));
    ...
    public function itIgnoresWhenStillReturnManifested(): void
    {
        ...
        $shipment = ShipmentTestFactory::new()
            ...
            ->create();
        $this->store($shipment);
        $reconciler = new ReturnManifestedShipmentReconciler(new StubCarrierGateway([$returnTrackingReference => ShipmentStatus::RETURN_MANIFESTED->value]), $this->service(CommandBusInterface::class));
