<?php

declare(strict_types=1);

namespace Shopping\Tests\Checkout\Domain\ValueObject;

use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\Attributes\Test;
use PHPUnit\Framework\TestCase;
use Shopping\Checkout\Domain\ValueObject\Email;

final class EmailTest extends TestCase
{
    #[Test]
    public function itCreates(): void
    {
        // When
        $email = Email::fromString('shopper@example.com');

        // Then
        self::assertSame('shopper@example.com', $email->value);
    }

    #[Test]
    #[DataProvider('provideInvalidValues')]
    public function itProtectsInvariants(string $value): void
    {
        // Then
        $this->expectException(\InvalidArgumentException::class);

        // When
        Email::fromString($value);
    }

    /**
     * @return iterable<string, array{string}>
     */
    public static function provideInvalidValues(): iterable
    {
        yield 'empty string' => [''];
        yield 'whitespace only' => ['   '];
        yield 'missing at sign' => ['not-an-address'];
        yield 'missing domain' => ['shopper@'];
        yield 'missing local part' => ['@example.com'];
    }

    #[Test]
    public function itEquals(): void
    {
        // Given
        $a = Email::fromString('shopper@example.com');
        $b = Email::fromString('  Shopper@Example.COM  ');

        // When
        $equals = $a->equals($b);

        // Then
        self::assertTrue($equals);
    }

    #[Test]
    public function itDiffers(): void
    {
        // Given
        $a = Email::fromString('shopper@example.com');
        $b = Email::fromString('other@example.com');

        // When
        $equals = $a->equals($b);

        // Then
        self::assertFalse($equals);
    }
}
